...
def run_face(fm, im):
    ...
    # 脸框（归一化）：全关键点包围盒 → 顶部外扩盖发髻/盔帽（底部到下颚）
    xs = [p.x for p in lm]
    ys = [p.y for p in lm]
    x0, x1, y0, y1 = min(xs), max(xs), min(ys), max(ys)
    h = y1 - y0
    bbox = [max(0.0, x0 - 0.25 * h), max(0.0, y0 - 0.75 * h),
            min(1.0, x1 + 0.25 * h), min(1.0, y1)]
    return d, round(frx, 3), bbox
