...
| 漏检（None / ±1.0 / 异常码） | — | 🔴 **硬失败，人眼不得改判**（2026-08-28 用户重申：判定不了 = 按失败计；唯一出路 = 换 seed 重抽让数据出现） |
| 数值在场但贴线（脸 0.025~0.029 等） | — | 人眼可终审改判（阿市 0.029 / 信长 0.028 已裁定"对"） | 设计如此 |
...
## 11. 底图判向参数快照（2026-08-29，改判向前的"当前参数"，供回滚对照）

| 项 | 当前值 | 说明 |
|---|---|---|
| 判向来源 | `verify_pose.judge()['face_dir']`（**仅脸**：鼻偏 ±0.003 带） | build_refs_full.py 用 face-only 判向 |
| 阈值带 | ±0.003（RIGHT/LEFT/FRONT） | 与生成闸门 0.030 无关，纯分类 |
| 镜像规则 | LEFT → PIL 翻转存 `_朝右`；RIGHT → `_朝右`；FRONT → `_朝正`；None → `_朝正` | enforce_right.py 强执 0 朝左 |
| 朝正处理 | 保留为中性锚（正脸镜像=仍正，无法变侧脸） | 109 张 |
| 复用规则 | 已有 `_朝右/_朝正` 同名文件直接复用（排除 `_朝左`） | build_refs_full 内存缓存 |
| 别名通道 | CNName + ScriptName + Name_* 年份列 + NAME_ALIAS 反向（gen_entity_maps，7 条） | 1001/1047 命中 |
| 分布 | 朝右 899 / 朝正 109 / 无底图 39 | manifest = hero_refs_manifest.json |
| 用户指认问题（2026-08-29） | 17 张"脸判向对但**身体朝向**错"：1288_海雷丁/1287_三浦按针/1279_三枝昌贞/1276_苏我理右卫门/1246_有马丰氏/1232_甘粕景继/685_曲直濑道三/683_松浦镇信/664_真壁氏干/617_福留仪重/596_原虎胤/585_蜂须贺小六/557_南部晴政/158_大熊朝秀/167_大友宗麟/119_上杉谦信 等 | 判向待改：见 §12 |
