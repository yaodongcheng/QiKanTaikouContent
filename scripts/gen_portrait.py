...
def _load_config():
    try:
        p = os.path.join(os.getcwd(), 'api_config.json')
...
# ---------- 脸素材池构图（2026-09-01：换脸管线专用，与立绘构图相反——要正脸直视镜头） ----------
def composition_face():
    return (
        '画面为中年武将的头部正面特写：构图从头顶至锁骨，脸部占据画面高度约百分之八十五；'
        '面部完全正对镜头，左右严格对称，双眼平视镜头且睁大，眼神直视；'
        '嘴巴紧闭、表情沉静；光源为正面柔和光，脸上无强烈投影、无侧光镂空；'
        '不戴头盔、冠帽、面具，脸上无任何遮挡物（颌下月牙须、颊侧短髭完整保留）；'
        '纯深棕渐变背景。'
    )

NEG_FACE = NEG.replace('正面直视镜头、', '')
