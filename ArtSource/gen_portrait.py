...
STYLE = (
    '日本光荣公司现代武将立绘CG画风：写实厚涂数字绘画，笔触细腻专业；'
...
REF_HINT = (
    '以上参考图仅提供该人物的面部骨相、发型须发与标志性服饰的底稿；'
...
def composition():
    return (
        '竖幅单人半身武将立绘，下沿切在腰胯之间，不画腿脚。'
        '人物身体与头部作为一个整体向画面右侧旋转：'
        '鼻梁与下颌剪影朝向画面右方，镜头只能看到半边脸颊与一只眼睛；'
        '画面左侧的肩膀位于近景，体积大而完整；'
...
SPIRIT_FACE = {'勇敢': '目光炯炯有神，下颌绷紧', '胆小': '眼神谨慎游移，肩略收',
               '普通': '神情平和'}
WEAPON = {'枪': '手边或身后可见长枪枪柄一截', '刀剑': '腰间打刀清晰可见',
          '弓': '身后背弓', '铁炮': '手扶火绳铁炮枪身'}
...
def character_layer(h, include_appearance=True):
    ...
    if app:
        ...
        weapon = WEAPON.get(h.get('weapon', ''), '') or ''
        return '。'.join(['%d 岁的日本战国时代%s' % (int(h['age']), who), weapon,
                          app,
                          '发型、年龄感、表情与服饰以上述形象描述为准，'
                          '禁止另加乌帽子、头盔、银发老态或与描述冲突的特征。']) + '。'
    if h.get('gender') == '女':
        ident = h['identity']
        dress = FEMALE_DRESS.get(ident, FEMALE_DRESS['公主'])
        parts = ['%d 岁的日本战国时代女性' % int(h['age']), dress, FEMALE_FACE,
                 WEAPON.get(h.get('weapon', ''), '') or '腰佩细刀']
        if include_appearance and h.get('appearance'):
            parts.append(h['appearance'])
        return '。'.join(p for p in parts if p) + '。'
    ident = h['identity']
    dress = IDENTITY_DRESS.get(ident, IDENTITY_DRESS['无效'])
...
def build_prompt(h, style_boost=False, has_ref=False, include_appearance=True):
    style = STYLE + ('严格遵守写实厚涂CG画风，坚决禁用任何浮世绘、水墨、工笔或扁平插画倾向。'
                     if style_boost else '')
    segs = [style, character_layer(h, include_appearance), composition(), NEG]
    if has_ref:
